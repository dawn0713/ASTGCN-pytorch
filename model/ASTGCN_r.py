...
class ASTGCN_submodule(nn.Module):
    ...
    def __init__(self, DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials,
        ...
        self.W = nn.Parameter(torch.FloatTensor(num_of_vertices, num_for_predict).to(DEVICE))
...
class ASTGCN(nn.Module):
    ...
    def __init__(self, DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, adj_mx,
                 num_for_predict, len_input, num_of_vertices):
        super(ASTGCN, self).__init__()

        L_tilde = scaled_Laplacian(adj_mx)
        cheb_polynomials = [torch.from_numpy(i).type(torch.FloatTensor).to(DEVICE) for i in cheb_polynomial(L_tilde, K)]
        self.submodule = nn.ModuleList([ASTGCN_submodule(DEVICE, nb_block, in_channels, K, nb_chev_filter,
                                                         nb_time_filter, time_strides, cheb_polynomials,
                                                         num_for_predict, len_input, num_of_vertices)])
        self.submodule.append(
            ASTGCN_submodule(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
        ...
        # self.model = ASTGCN_submodule(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
        #                          cheb_polynomials, num_for_predict, len_input, num_of_vertices)
    ...
    def forward(self, x_list):
        ...
        y1 = self.submodule[0](x_list[0])
        y2 = self.submodule[1](x_list[1])
        y3 = self.submodule[2](x_list[2])
